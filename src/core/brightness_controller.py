...
(Brightness Controller - Brightness control logic)
"""
...
class BrightnessController:
    ...
    def sync_mode(self) -> bool:
        """Sync mode có được bật không (Whether sync mode is enabled)"""
    ...
    def toggle_sync_mode(self, enabled: bool):
        ...
        Args:
            enabled: True để bật, False để tắt (True to enable, False to disable)
        """
        with self._lock:
            ...
            # (If enabling sync mode, sync all monitors to global brightness)
            if enabled:
    ...
    def set_global_brightness(self, value: int):
        ...
        (Set brightness for ALL monitors - sync mode)
    ...
    def _set_all_brightness_internal(self, value: int, save_global: bool = True):
        ...
        (Internal method to set brightness for all monitors)
        
        Args:
            value: Độ sáng (Brightness)
            save_global: Có lưu vào global_brightness không
                        (Whether to save to global_brightness)
        """
    ...
    def set_monitor_brightness(self, monitor_id: str, value: int):
        ...
        (Set brightness for specific monitor - individual mode)
    ...
    def restore_last_brightness(self):
        ...
        (Restore brightness from config on startup)
        """
        with self._lock:
            monitors = self.monitor_manager.get_monitors()
